...
def calculate_volatility(data: pd.DataFrame, days: int):
    ...
    return (
        data.groupby("symbol")["return"]
        .transform(lambda x: x.rolling(window=days, min_periods=int(days * 0.8)).std())
        .round(6)
    )


def calculate_annualized_volatility(data: pd.DataFrame, days: int):
    """Calculate annualized standard deviation of daily return"""
    return (calculate_volatility(data, days) * np.sqrt(252)).round(6)


def calculate_downside_volitility(data: pd.DataFrame, days: int):
    """Calculate negative reutrn's standard deviation"""
    if "return" not in data.columns:
        data["return"] = calculate_return(data)
    data["negative_return"] = data["return"].clip(upper=0)
    return (
        data.groupby("symbol")["negative_return"].transform(
            lambda x: x.rolling(window=days, min_periods=int(days * 0.8)).std()
        )
        * np.sqrt(252)
    ).round(6)
...
def calculate_avg_volume(data: pd.DataFrame, days: int):
    ...
    return (
        ...
        .transform(lambda x: x.rolling(window=days).mean())
        .round(0)
    )
...
def calculate_median_volume(data: pd.DataFrame, days: int):
    ...
    return (
        data.groupby("symbol")["volume"]
        .transform(lambda x: x.rolling(window=days).median())
        .round(0)
    )


def calculate_dollar_volume(data: pd.DataFrame):
    """Calculate stock close price times trading volume"""
...
def calculate_avg_dollar_volume(data: pd.DataFrame, days: int):
    ...
    return (
        data.groupby("symbol")["dollar_volume"]
        .transform(lambda x: x.rolling(window=days).mean())
        .round(2)
    )
...
def calculate_amihud(data: pd.DataFrame, days: int):
    ...
    return (
        data.groupby("symbol")["amihud_daily"]
        .transform(lambda x: x.rolling(window=days, min_periods=int(days * 0.8)).mean())
        .round(10)
    )
...
def calculate_ema(data: pd.DataFrame, days: int):
    ...
    return (
        data.groupby("symbol")["close_price"]
        .transform(lambda x: x.ewm(span=days).mean())
        .round(4)
    )
...
def calculate_adx(data: pd.DataFrame, days: int = 14):
    ...
    def _compute_adx_per_symbol(group):
        ...
        plus_di = (
            100 * pd.Series(plus_dm).ewm(alpha=1 / days, adjust=False).mean() / atr
        )
        minus_di = (
            100 * pd.Series(minus_dm).ewm(alpha=1 / days, adjust=False).mean() / atr
        )
...
def calculate_donchian_high(data: pd.DataFrame, days: int):
    ...
    return (
        data.groupby("symbol")["high_price"]
        .transform(lambda x: x.rolling(window=days).max())
        .round(4)
    )
...
def calculate_donchian_low(data: pd.DataFrame, days: int):
    ...
    return (
        data.groupby("symbol")["low_price"]
        .transform(lambda x: x.rolling(window=days).min())
        .round(4)
    )
...
def calculate_price_to_weeks_high(data: pd.DataFrame, days: int = 252):
    """Calculate the ratio of current price to yearly high"""
    rolling_52w_high = data.groupby("symbol")["high_price"].transform(
...
def calculate_ma_roc(data: pd.DataFrame, ma_days: int, window_days: int):
    """Calculate the change rate of moving average"""
...
def calculate_lagged_momentum(
    data: pd.DataFrame, total_months: int, lag_months: int = 1
):
...
def calculate_risk_adj_momentum(
    data: pd.DataFrame, momentum_months: int, vol_days: int
):
    """Calculate lagged return divided by volatility"""
...
def calculate_risk_adj_return(data: pd.DataFrame, return_months: int, vol_days: int):
    """Calculate annualized return divided by volatility"""
    if "annualized_return" not in data.columns:
        data["annualized_return"] = calculate_annualized_return(
            data, return_months * 21
        )
    if "volatility" not in data.columns:
        data["volatility"] = calculate_annualized_volatility(data, vol_days)
...
def calculate_positive_return_percent(data: pd.DataFrame, days: int):
    """Calculate the percentage of positive return within the window"""
    data["is_positive"] = (calculate_return(data) > 0).astype(int)
    return (
        data.groupby("symbol")["is_positive"]
        .transform(lambda x: x.rolling(window=days, min_periods=int(days * 0.8)).mean())
        .round(6)
    )
...
def calculate_maximum_drawdown(data: pd.DataFrame, days: int):
    """Calculate the maximum price drop wintin the window"""
    rolling_peak = data.groupby("symbol")["close_price"].transform(
        lambda x: x.rolling(window=days, min_periods=1).max()
    )
    data["raw_drawdown"] = (data["close_price"] - rolling_peak) / rolling_peak
    return (
        data.groupby("symbol")["raw_drawdown"]
        .transform(lambda x: x.rolling(window=days, min_periods=1).min())
        .round(6)
    )
...
def calculate_historical_var(
    ...
    """Calcualte the value at risk (VaR) base on historical data"""
    data[f"return_{days}d"] = calculate_return(data, days)
    alpha = 1 - confidence_level
    rolling_var_pct = data.groupby("symbol")[f"return_{days}d"].transform(
        lambda x: x.rolling(
            window=rolling_window, min_periods=int(rolling_window * 0.8)
        ).quantile(alpha)
    )
...
def calculate_historical_cvar(
    ...
    """Calculate the conditional value at risk (CVaR) base on historical data"""
    data[f"return_{days}d"] = calculate_return(data, days)
    alpha = 1 - confidence_level
    ...
    rolling_cvar_pct = data.groupby("symbol")[f"return_{days}d"].transform(
        lambda x: x.rolling(
            window=rolling_window, min_periods=int(rolling_window * 0.8)
        ).apply(_get_cvar)
    )
...
def calculate_worst_day_return(data: pd.DataFrame, days: int = 252):
    """Calculate the worst daily return within the window"""
    if "return" not in data.columns:
        data["return"] = calculate_return(data)
    return (
        data.groupby("symbol")["return"]
        .transform(lambda x: x.rolling(window=days, min_periods=int(days * 0.8)).min())
        .round(6)
    )


def calculate_worst_week_return(data: pd.DataFrame, days: int = 252):
    """Calculate the worst weekly return (5 days) within the window"""
    data["weekly_return"] = calculate_return(data, 5)
    return (
        data.groupby("symbol")["weekly_return"]
        .transform(lambda x: x.rolling(window=days, min_periods=int(days * 0.8)).min())
        .round(6)
    )
...
def calculate_rsi(data: pd.DataFrame, days: int):
    ...
    avg_gain = gain.groupby(data["symbol"]).transform(
        lambda x: x.ewm(alpha=1 / days, adjust=False).mean()
    )
...
def calculate_bollingner(data: pd.DataFrame, days: int = 20, num_std: int = 2):
    """Calculate bollingner bands percentage (%B)"""
    mid_band = data.groupby("symbol")["close_price"].transform(
        lambda x: x.rolling(window=days).mean()
    )

    std_dev = data.groupby("symbol")["close_price"].transform(
...
def get_latest_indicators(symbols: list, as_of_date: str):
    ...
    final_merged_df["rar_rank"] = final_merged_df["risk_adj_mom_12m"].rank(
        ascending=True, pct=True, na_option="keep"
    )
    final_merged_df["stability_rank"] = final_merged_df["positive_ret_pct_60d"].rank(
        ascending=True, pct=True, na_option="keep"
    )
    final_merged_df["momentum_score"] = (
        0.7 * final_merged_df["rar_rank"] + 0.3 * final_merged_df["stability_rank"]
    )
